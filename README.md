...
**Same trade-off — no workspace writes.** Because servers run as `devbox-mcp` and not as your agent user `node`, they cannot write to the workspace (owned by `node`) or read your private `0600` project files. API-based servers like `context7` and `taskmaster-ai` are unaffected, but a **filesystem-type MCP server that needs to modify your project is not supported** in this version — running it as `node` to grant that access would re-expose its secrets to the agent. Closing this is the same future decision as peer isolation (per-server UIDs).
